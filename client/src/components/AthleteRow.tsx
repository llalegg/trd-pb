...
import { ChevronDown, ChevronRight, AlertTriangle, Activity, AlertCircle, Eye, Edit, FileCheck, Plus, CheckCircle, Check, CheckCircle2, ChevronUp } from "lucide-react";
...
interface AthleteRowProps {
  ...
  isExpanded: boolean;
  onToggleExpand: () => void;
  matchingBlockIds?: Set<string>; // IDs of blocks that matched the current filters
}
...
const getAvatarBorderClass = (status?: "injured" | "rehabbing" | "lingering-issues" | null): string => {
  // Remove colored borders - status shown via icon only
  return "";
};

const getBlockStatusBadge = (status: Block["status"]) => {
  const variants: Record<Block["status"], { label: string; className: string; icon?: React.ReactNode }> = {
    active: { label: "Active", className: "bg-green-500/20 text-green-400 border-green-500/30" },
    complete: { 
      label: "Complete", 
      className: "bg-[#979795]/5 text-[#979795] border-transparent",
...
const getBlockProgressText = (block: Block): string | null => {
  // Only show progress text for active blocks
  if (block.status !== "active") {
    return null;
  }
  
  // For active blocks, show progress
  // Prefer currentDay format if available
  if (block.currentDay) {
    return `Week ${block.currentDay.week}, Day ${block.currentDay.day}`;
  }
  
  // Fallback to days format
  if (block.daysComplete !== undefined && block.daysAvailable !== undefined) {
    return `Day ${block.daysComplete} of ${block.daysAvailable}`;
  }
...
const getBlockProgressPercent = (block: Block): number => {
  if (block.status !== "active" || block.daysComplete === undefined || block.daysAvailable === undefined) {
    return 0;
  }
  
  if (block.daysAvailable === 0) return 0;
  return Math.min(100, Math.max(0, (block.daysComplete / block.daysAvailable) * 100));
};

const getSeasonBadgeStyle = (season: string): string => {
  // Neutral: no background colors, white text
  return "bg-transparent border-[#292928] text-[#f7f6f2]";
};

const getSeasonDisplayText = (season: string, subSeason?: string): React.ReactNode => {
  if (subSeason) {
    return (
      <>
        {season} <span className="text-[#979795]">({subSeason})</span>
      </>
    );
  }
  return season;
...
const formatPhaseTimeline = (currentPhase?: Phase): string | null => {
  if (!currentPhase || currentPhase.blocks.length === 0) return null;
  
  const sortedBlocks = [...currentPhase.blocks].sort((a, b) => 
    new Date(a.startDate).getTime() - new Date(b.startDate).getTime()
  );
  ...
  const start = new Date(firstBlock.startDate);
  const end = new Date(lastBlock.endDate);
  
  // Format: "Oct 28 - Dec 23, 2025 • 8 weeks"
...
const getCurrentBlockStatus = (blocks: Block[]): { text: React.ReactNode; color: string } => {
  ...
  if (activeBlock) {
    const season = activeBlock.season || activeBlock.name.replace(/Block \d+[: ]?/i, '').trim();
    return { 
      text: (
        <>
          <span className="text-[#f7f6f2]">{season} Block {activeBlock.blockNumber}</span>
          <Badge variant="outline" className="text-xs font-['Montserrat'] flex items-center gap-1 bg-green-500/20 text-green-400 border-green-500/30">
            Active
          </Badge>
        </>
      ),
      color: "" // No color class needed, handled in JSX - always return empty string
    };
  }
  
  return { 
    text: "No active block", 
    color: "text-[#979795]" // Always return a string
  };
};
...
const getNextAction = (blocks: Block[]): { text: string | null; urgency: "overdue" | "today" | "thisWeek" | "later" | null } => {
  ...
  // Don't show "Sign-off required" - the amber border and badge indicate this
  // Check for next block due
  ...
  if (nextBlockDue?.nextBlockDue) {
    const dueDate = new Date(nextBlockDue.nextBlockDue);
    ...
    if (diffDays < 0) {
      text = `Block due ${Math.abs(diffDays)}d ago`;
      urgency = "overdue";
    } else if (diffDays === 0) {
      text = "Block due today";
      urgency = "today";
    } else if (diffDays === 1) {
      text = "Block due tomorrow";
      urgency = "today";
    } else if (diffDays <= 7) {
      text = `Block due in ${diffDays} days`;
      urgency = "thisWeek";
    } else {
      text = `Block due in ${diffDays} days`;
      urgency = "later";
...
const getNextActionColor = (urgency: "overdue" | "today" | "thisWeek" | "later" | null): string => {
  switch (urgency) {
    ...
    case "thisWeek":
      return "text-amber-500"; // Changed from orange-400 to amber-500
    case "later":
      return "text-[#979795]";
    default:
      return "text-[#979795]";
...
export default function AthleteRow({ athleteData, isExpanded, onToggleExpand, matchingBlockIds = new Set() }: AthleteRowProps) {
  ...
  const nextActionColor = typeof nextActionUrgency === "string" || nextActionUrgency === null 
    ? getNextActionColor(nextActionUrgency) 
    : "";
  ...
  return (
    <div className={cn(
      "bg-[#1a1a19] border border-[#292928] rounded-lg px-4 pt-4 pb-4 mb-6 transition-all duration-200 hover:border-[#3a3a38] hover:shadow-lg"
    )}>
      {/* Athlete Summary Row */}
      <Tooltip>
        ...
        {!isExpanded && (
          <div className="flex-1 min-w-0 hidden md:flex md:justify-start">
            <div className={cn(
              "text-sm font-['Montserrat'] px-3 py-1.5 rounded-full border border-[#292928] bg-[#171716] text-[#979795] hover:bg-[#1a1a19] hover:border-[#3a3a38] transition-colors duration-200 flex items-center gap-2",
