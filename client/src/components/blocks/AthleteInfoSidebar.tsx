...
import { AlertTriangle, Activity, AlertCircle, PanelLeftOpen, PanelLeftClose } from "lucide-react";
...
export default function AthleteInfoSidebar({ athlete, currentPhase, blocks }: AthleteInfoSidebarProps) {
  ...
  return (
    <aside className={cn(
      "transition-all duration-200 border-r border-[#292928] bg-[#0d0d0c] flex-shrink-0",
      open ? "w-72" : "w-10"
    )}>
      <button
        onClick={() => setOpen(v => !v)}
        aria-label={open ? "Collapse sidebar" : "Expand sidebar"}
        className="w-full h-10 flex items-center justify-center border-b border-[#292928] hover:bg-[#171716] transition-colors"
      >
        {open ? <PanelLeftClose className="w-4 h-4 text-[#979795]" /> : <PanelLeftOpen className="w-4 h-4 text-[#979795]" />}
      </button>
      {open && (
